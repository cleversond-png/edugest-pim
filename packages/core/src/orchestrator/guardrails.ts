...
// Resolve schema directory: look for 'schemas/' from cwd, or up one/two levels (monorepo structure)
function getSchemaDir(): string {
  ...
  // Fallback: still return the first candidate (will error if schemas not found)
  return candidates[0];
}
